...
with st.sidebar:
    ...
    if os.path.exists(DB_FILE):
        with open(DB_FILE, 'rb') as f:
            # 修改了文件名格式，加入秒，方便在 Google Drive 里排序
            now_str = datetime.now().strftime('%Y年%m月%d日_%H时%M分%S秒')
            st.download_button(
                label="🚨 点击下载备份 (存入 Google Drive)",
                data=f,
                file_name=f"我的知识库备份_{now_str}.csv",
                mime="text/csv"
            )
...
def process_content(text, key):
    ...
    任务：
    1. 从以下 8 个精准标签中选一个最合适的：
       【💰 财富理财】、【📈 商业思维】、【🏋️ 运动健身】、【🥗 饮食营养】、
       【🎭 心理人性】、【💬 社交情商】、【🚀 自我成长】、【📥 杂项收件箱】。
    2. 起一个简短标题。
    3. 提炼 3 点实操干货。
...
if api_key:
    ...
    try:
        ...
        if len(df_all) > 0:
            ...
            cats = ["财富", "商业", "健身", "营养", "人性", "社交", "成长"]
            for i in range(7):
                with tabs[i]:
                    st.table(df_all[df_all['分类'].str.contains(cats[i])][["时间", "标题", "精华内容"]])
            with tabs[7]:
